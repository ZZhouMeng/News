...
<main class="col-md-8 main-content">
	...
	<% 	int pageSize=5;
		...
		sql = "select top "+pageSize+"* from Content where category='"+category+"' and id not in (select top "+x+" id  from Content where category='"+category+"' order by id desc) order by id desc";
	...
	<nav class="pagination" role="navigation">
		<%
		 ...
		 %>
        <a class="newer-posts" href="category.jsp?category=<%=ids %>&page=<%=pages-1%>"><i class="fa fa-angle-left"></i></a>
        <%} %>
    <span class="page-number">第 <%=pages %>页 &frasl; 共 <%=num %> 页</span>
    	<%
		 ...
		 %>
        <a class="older-posts" href="category.jsp?category=<%=ids %>&page=<%=pages+1%>"><i class="fa fa-angle-right"></i></a>
        <%} %>
